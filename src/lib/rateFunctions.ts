...
export async function getRelevantFoldersFromAi(content: string[]){
    ...
    const instruction = 'as i am making a development project rater i am using u as an agent whose job is to just to output just an array of top 2 - 3 folders and files which you think would be good to look into recursively, make sure they should be less as the context cannot be very big to rate make sure to give less files and only the most relevant one avoid unnecessary files for example package.json can be relevant but config files are of no use similarly files having some code written in them components can be useful just be very picky as you have context which will be good to view into to look into from the given folder structure, you dont have to write anything else just analyze and give me an object with key 1 as relevantFiles: string[] and key2 will be relevantFolders: string[] as simple as that you will get the content after the colon: '
